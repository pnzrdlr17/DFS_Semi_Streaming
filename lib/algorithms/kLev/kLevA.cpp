// without one pass heuristic + with marked/unmarked + with nk space correction (backedges thing)
// with total nk space (H2) + without top path
...
class kLevA : public kLevBase { // only H2
...
public:
    ...
    kLevA (int size, int space_optimality) {
        n = size+1;
        pass = 0;
        k = space_optimality;
