...
class kLevB : public kLevBase { // only H3 copied from kLev0
...
public:
    ...
    kLevB (int size, int space_optimality) {
        n = size+1;
        pass = 0;
        k = space_optimality;
        ...
        if(pass == 1){
            ...
            while(!queue.empty()){
                ...
                if(lca == y){
                    ...
                    !(T.par(anc_x) != -1 && is_stared[T.par(anc_x)] == 1))
                ...
                if(LA.level(anc_x) <= k && marked[anc_x] == 0 && !(T.par(anc_x) != -1 && is_stared[T.par(anc_x)] == 1)){
