// kpath with one pass heuristic + with nk space correction + with no duplicates + with optimal_k
...
public:
    ...
    void post_processing(int compN) {
        ...
        compL[compN].clear();
        // sptree[compN].clear(); //(sri) changed
    ...
    void processComp(int compN) {
        ...
        for(auto a: comp_list){
            ...
            compL[a].push_back(a);
            processed[a] = 1;
            if(!sptree[a].empty()) sptree[a].clear(); //(sri) not needed
        }
    ...
    void addEdge(int x, int y) {
        ...
        {
            if(sptree[comp[x]].find({x, y}) == sptree[comp[x]].end()) saved_edges[comp[x]].push_back({x,y}); // changed here.
            if(saved_edges[comp[x]].size()+sptree[comp[x]].size() >= compL[comp[x]].size()*k_opt) // changed here
            {
                processComp(comp[x]);
            }
        }
