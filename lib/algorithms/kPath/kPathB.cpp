...
class kPathB : public kPathBase{ // only H3, copied from kPath0 and used k_opt
    int k_opt;
    vector<list<edg>> sptree;

public:

    kPathB (int size,int spaceOpt) {
        n = size+1;
    ...
    void addEdge(int x, int y) {
        ...
        else if(visited[x]==0 && visited[y]==0)
        {
            ...
            if(saved_edges[comp[x]].size() >= compL[comp[x]].size()*(k_opt - 1)) { // used k_opt instead of sopt // H3 change
                processComp(comp[x]);
            }
        }
    }
