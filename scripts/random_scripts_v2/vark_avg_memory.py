...
def run_experiments(iterations, seed_token, seed_file, sparsity, graph_type):
    ...
    k_values = list(range(1, 10, 1)) + list(range(10, 20, 2)) + list(range(20, 50, 5)) + list(range(50, 100, 10)) + list(range(100, 300, 50)) + list(range(300, 500, 100)) + list(range(500, 1000, 250)) + list(range(1000, 3000, 500)) + list(range(3000, 5000, 1000)) + list(range(5000, 10001, 2500))
    ...
    for algorithm_name, algorithm_code in algorithms.items():
        ...
        for variant in variants:
            csv_filename = f"{algorithm_name}{variant}_{sparsity_label[sparsity]}_{run_id}.csv"
            csv_filepath = os.path.join(dir_path, csv_filename)

            # Open CSV file and write header
            csvfile = open(csv_filepath, "w", newline="", buffering=1)
            csvwriter = csv.writer(csvfile, delimiter=',')
            csvwriter.writerow(["K", "Time (s)", "Memory (KB)", "AvgPasses", "AvgHeight"])
