...
def run_experiments(iterations, seed_token, output_dir):
    ...
    run_output_dir = os.path.join(output_dir, f"run_random_powerlaw_{timestamp}_seed_{seed_token}_iter_{iterations}")
    ...
    for sparsity in sparsity_values:
        ...
        for n in n_values:
            ...
            try:
                subprocess.run(
                    ["./bin/main", "PREP_EXP", "3", str(n), str(sparsity), "POWLAW", str(iterations), str(seed_token)],
                    check=True
                ...
                for variant in variants:
                    ...
                    try:
                        result = subprocess.run(
                            ["/usr/bin/time", "-f", "%U,%M", "./bin/main", "RUN_EXP", "3", str(n), str(sparsity), "POWLAW", str(iterations), str(seed_token), algorithm_code, variant, str(k)],
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-o", "--output-dir",
        default="./results/random/powerlaw_figure_5",
        help="Directory to save the results (default: ./results/random/powerlaw_figure_5)"
    )
