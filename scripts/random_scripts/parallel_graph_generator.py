#!/usr/bin/env python3
"""
Parallel Graph Generator Script
...
def get_varn_values() -> List[int]:
    """Extract n values from VARN script"""
...
def get_varm_values() -> List[int]:
    """Extract m values from VARM script (with fixed n=10000)"""
...
def generate_single_graph(n: int, m: int, seed: int, graph_type: str) -> Tuple[bool, str]:
    """Generate a single graph file using the main binary"""
    try:
        ...
        if result.returncode == 0:
            ...
            return True, f"Generated graph n={n}, m={m}, seed={seed}, type={graph_type} -> {generated_file}"
        else:
            return False, f"Error generating graph n={n}, m={m}, seed={seed}: {result.stderr}"
            
    except subprocess.TimeoutExpired:
        return False, f"Timeout generating graph n={n}, m={m}, seed={seed}"
