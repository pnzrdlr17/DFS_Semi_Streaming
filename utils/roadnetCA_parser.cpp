 ...
 * 2. Use the `roadNet-CA.txt` file directly as input to this program.
 * 3. Compile the program using the following command:
 *      g++ -o roadnetCA_parser roadnetCA_parser.cpp
 * 4. Run the program with the path to the CSV file as an argument:
 *      ./roadnetCA_parser <path_to_roadNet-CA.txt>
 * 5. OR compile and run at once using:
 *      g++ -o roadnetCA_parser roadnetCA_parser.cpp && ./roadnetCA_parser <path_to_roadNet-CA.txt>
...
/*
    ...
    * 1. Contains N=1965206 nodes
    * 2. Contains M=2766607 edges (unique)
    * 3. No self-loops found
    * 4. No duplicate edges found
    ! 5. Nodes labelled from 0 to 1971280 (instead of 1 to 1965206)
        ? This is fixed by the below code and the output file has the nodes labelled from 1 to 1965206.
    ! 6. The graph has duplicate for each edge, so the actual number of edges is 2766607, but the file contains 5533214 edges
        ? Only one of the duplicate edges is considered while generating the output file, so M=2766607.
...
const ll N = 1965206; // Number of nodes
const ll M = 2766607; // Number of edges (unique)
...
void generateOutputFile(ifstream& infile) {
    string outputFileName = "output_roadnetCA_graph.edg";
