...
for input_data in "${inputs[@]}"; do
    ...
    # Iterate over all the variants (0, 1, 2, N, X, Y) for kLev and kPath
    for variant in 0 1 2 N X Y; do
        ...
        for K in {1..10}; do
            ...
            for i in $(seq 1 $iterations); do
                ...
                result=$(gtime -f "%U,%M" ./bin/main_REAL "$N" "$M" "$INPUT_PATH" 3 0 "$K" "$variant" 2>&1) # 3 for KLev
